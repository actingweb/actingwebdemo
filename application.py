...
def get_config():
    ...
    return config.Config(
        ...
        ui=True,
        bot={"token": bot_token, "email": bot_email, "secret": bot_secret, "admin_room": bot_admin_room},
        oauth=oauth,
    )
...
class Handler:
    def __init__(self, req):
        ...
        self.webobj = aw_web_request.AWWebObj(
            url=req.url, params=req.values, body=req.data, headers=req.headers, cookies=req.cookies
        )
        if not req or not self.path:
            return
        if self.path == "/":
            self.handler = factory.RootFactoryHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
        else:
            path = self.path.split("/")
            self.path = path
            f = path[1]
            if f == "oauth":
                self.handler = callback_oauth.CallbackOauthHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
            elif f == "bot":
                self.handler = bot.BotHandler(webobj=self.webobj, config=get_config(), on_aw=OBJ_ON_AW)
            elif len(path) == 2:
                self.handler = root.RootHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
            else:
                self.actor_id = f
                f = path[2]
                if f == "meta":
                    # r'/<actor_id>/meta<:/?><path:(.*)>'
                    self.handler = meta.MetaHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                elif f == "oauth":
                    # r'/<actor_id>/oauth<:/?><path:.*>'
                    self.handler = oauth.OauthHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                elif f == "www":
                    # r'/<actor_id>/www<:/?><path:(.*)>'
                    self.handler = www.WwwHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                elif f == "properties":
                    # r'/<actor_id>/properties<:/?><name:(.*)>'
                    self.handler = properties.PropertiesHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                elif f == "trust":
                    # r'/<actor_id>/trust<:/?>'
                    # r'/<actor_id>/trust/<relationship><:/?>'
                    # r'/<actor_id>/trust/<relationship>/<peerid><:/?>'
                    if len(path) == 3:
                        self.handler = trust.TrustHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                    elif len(path) == 4:
                        self.handler = trust.TrustRelationshipHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                    elif len(path) >= 5:
                        self.handler = trust.TrustPeerHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                    ...
                    if len(path) == 3:
                        self.handler = subscription.SubscriptionRootHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                    elif len(path) == 4:
                        self.handler = subscription.SubscriptionRelationshipHandler(
                            self.webobj, get_config(), on_aw=OBJ_ON_AW
                        )
                    elif len(path) == 5:
                        self.handler = subscription.SubscriptionHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                    elif len(path) >= 6:
                        self.handler = subscription.SubscriptionDiffHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                elif f == "callbacks":
                    # r'/<actor_id>/callbacks<:/?><name:(.*)>'
                    self.handler = callbacks.CallbacksHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                elif f == "resources":
                    # r'/<actor_id>/resources<:/?><name:(.*)>'
                    self.handler = resources.ResourcesHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
                elif f == "devtest":
                    # r'/<actor_id>/devtest<:/?><path:(.*)>'
                    self.handler = devtest.DevtestHandler(self.webobj, get_config(), on_aw=OBJ_ON_AW)
        if not self.handler:
    ...
    def get_response(self):
        if self.webobj.response.redirect:
            ...
            if self.webobj.response.status_code == 401:
                ...
                self.webobj.response.headers["WWW-Authenticate"] = 'Basic realm="' + (self.actor_id or "") + '"'
...
def app_www(actor_id, path=""):
    ...
    if not h.process(actor_id=actor_id, path=path):
        return Response(status=404)
    redirect_response = h.get_redirect()
